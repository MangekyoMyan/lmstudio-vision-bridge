/**
 * Chat history conversion: LM Studio -> OpenAI-compatible (prebuilt mirror of src/messages.ts).
 *  - `tool` messages: images are NEVER attached (SDK constraint); file parts
 *    become "[attached file: ...]" text notes.
 *  - assistant tool calls: both LM style and OpenAI style understood.
 */
...
export function fileToDataUrl(absPath, mimeTypeHint) {
  let buf;
  try {
    buf = fs.readFileSync(absPath);
  } catch (e) {
    throw new BridgeError("image_read_failed", `cannot read image file: ${absPath} — ${e.message}`);
  }
  ...
  if (typeof content === "string") {
    ...
    for (const raw of content) {
      ...
      if (allowImages) {
        ...
        const name =
          partString(o, "file_name") ?? partString(o, "fileName") ?? partString(o, "name") ?? partString(o, "path") ?? "file";
...
export function normalizeOpenAIMessage(m, index) {
  ...
  if (typeof content === "string") {
    ...
    for (const raw of content) {
      ...
      if (p.type === "image_url" || (iu && typeof iu === "object")) {
        const u = iu && typeof iu === "object" ? iu.url : undefined;
 ...
 */
export function describeMessageShape(m, index) {
  ...
  if (Array.isArray(c)) {
    desc.partTypes = c.map((p) =>
      p && typeof p === "object"
        ? (typeof p.type === "string" ? p.type : `object:${Object.keys(p).slice(0, 6).join("/")}`)
        : typeof p
    );
  ...
  if (Array.isArray(o.tool_calls)) desc.toolCalls = o.tool_calls.length;
  if (typeof o.tool_call_id === "string") desc.toolCallId = o.tool_call_id;
  return desc;
}

export function normalizeToolCallAny(r, index) {
...
export function extractAssistantToolCalls(m, index) {
  const out = [];
