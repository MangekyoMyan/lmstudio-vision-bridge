...
exports.normalizeOpenAIMessage = normalizeOpenAIMessage;
exports.normalizeOpenAIMessages = normalizeOpenAIMessages;
exports.describeMessageShape = describeMessageShape;
exports.normalizeToolCallAny = normalizeToolCallAny;
exports.extractAssistantToolCalls = extractAssistantToolCalls;
...
function guessMime(p) {
    return MIME_BY_EXT[node_path_1.default.extname(p).toLowerCase()] ?? "image/png";
}
function partString(p, key) {
...
function fileToDataUrl(absPath, mimeTypeHint) {
    let buf;
    try {
        buf = node_fs_1.default.readFileSync(absPath);
    }
    catch (e) {
        throw new types_js_1.BridgeError("image_read_failed", `cannot read image file: ${absPath} — ${e.message}`);
    }
...
function partImagePart(p, baseDir) {
    if (!p || typeof p !== "object")
        return null;
    ...
    const mime = ["mime_type", "mimeType", "mime", "content_type", "contentType"]
        .map((k) => partString(p, k))
        .find((v) => v !== null) ?? null;
    const isAbs = node_path_1.default.isAbsolute(fp) || /^[A-Za-z]:[\\/]/.test(fp);
    const candidates = isAbs ? [fp] : [
        ...(baseDir ? [node_path_1.default.join(baseDir, fp)] : []),
        node_path_1.default.resolve(process.cwd(), fp),
        fp,
    ];
    for (const c of candidates) {
        try {
            if (node_fs_1.default.existsSync(c) && node_fs_1.default.statSync(c).isFile()) {
                (0, log_js_1.dbg)("msg", `materialized file part as data URL`, { file: c });
...
function toOpenAIMessage(m, index, baseDir) {
    ...
    if (typeof content === "string") {
        ...
        for (const raw of content) {
            ...
            (0, log_js_1.dbg)("msg", `skipping unrecognized content part in message[${index}]`, Object.keys(o).slice(0, 12));
        }
    }
        ...
        if (t)
            ...
            const iu = o.image_url;
            if (iu && typeof iu === "object" && typeof iu.url === "string") {
                parts.push({ type: "image_url", image_url: { url: iu.url } });
            }
            else {
                const img = partImagePart(o, baseDir);
                if (img)
    ...
    if (role === "tool") {
        const id = partString(m, "tool_call_id") ?? partString(m, "toolCallId") ?? partString(m, "call_id") ?? `call_vb_t${index}`;
        out.tool_call_id = id;
    }
...
function normalizeOpenAIMessage(m, index) {
    if (!m || typeof m !== "object" || Array.isArray(m)) {
        (0, log_js_1.dbg)("msg", `message[${index}] is not a plain object; dropping from wire payload`, String(m));
        return null;
    }
    const o = m;
    if (typeof o.role !== "string") {
        (0, log_js_1.dbg)("msg", `message[${index}] has no string role; dropping from wire payload`, Object.keys(o).slice(0, 12));
        return null;
    }
    const role = (["system", "user", "assistant", "tool"].includes(o.role) ? o.role : "user");
    ...
    if (typeof content === "string") {
        ...
        for (const raw of content) {
            ...
            if (p.type === "image_url" || (iu && typeof iu === "object")) {
                ...
                (0, log_js_1.dbg)("msg", `message[${index}] dropping image part with missing/invalid url`, Object.keys(p).slice(0, 12));
                continue;
            }
            // text part / LM raw part: extract the text only
            pushText(extractTextOf(p));
            if (p.type !== undefined && p.type !== "text" && p.type !== "image_url") {
                (0, log_js_1.dbg)("msg", `message[${index}] dropping unsupported content part`, { partType: String(p.type), keys: Object.keys(p).slice(0, 12) });
            }
            else if (p.type === undefined && (p.path !== undefined || p.file_name !== undefined || p.fileName !== undefined || p.name !== undefined)) {
                (0, log_js_1.dbg)("msg", `message[${index}] dropping unmaterialized file part`, { name: p.name ?? p.file_name ?? p.fileName ?? p.path });
            }
        }
    }
        ...
        if (iu && typeof iu === "object" && typeof iu.url === "string") {
            imageParts.push({ type: "image_url", image_url: { url: iu.url } });
        }
        pushText(extractTextOf(content));
        (0, log_js_1.dbg)("msg", `message[${index}] content was a raw object; text extracted, object NOT forwarded`, Object.keys(o2).slice(0, 12));
    }
    else if (content === undefined || content === null) {
        (0, log_js_1.dbg)("msg", `message[${index}] content is ${String(content)}; normalizing to empty string`);
    }
    const out = { role, content: "" };
    if (role !== "user" && imageParts.length > 0) {
        (0, log_js_1.dbg)("msg", `message[${index}] (${role}) dropped ${imageParts.length} image part(s) — images are only allowed on user messages`);
    }
    if (role === "user" && imageParts.length > 0) {
    ...
    if (role === "assistant") {
        ...
        if (tcs.length > 0) {
            ...
            (0, log_js_1.dbg)("msg", `message[${index}] assistant tool call(s) were not normalizable; dropped`);
        }
    }
    if (role === "tool") {
...
function normalizeOpenAIMessages(messages) {
    if (!Array.isArray(messages)) {
        (0, log_js_1.warn)("msg", "normalizeOpenAIMessages: input was not an array; returning []");
        return [];
    }
    const out = [];
 ...
 */
function describeMessageShape(m, index) {
    ...
    if (Array.isArray(c)) {
        desc.partTypes = c.map((p) => p && typeof p === "object"
    ...
    if (Array.isArray(o.tool_calls))
        desc.toolCalls = o.tool_calls.length;
    if (typeof o.tool_call_id === "string")
        desc.toolCallId = o.tool_call_id;
    return desc;
}
/** Normalize either LM style or OpenAI style tool call objects. */
function normalizeToolCallAny(r, index) {
...
function extractAssistantToolCalls(m, index) {
    const out = [];
