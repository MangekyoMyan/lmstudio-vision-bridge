...
exports.VisionBridgeGenerator = void 0;
/**
...
class VisionBridgeGenerator extends BaseCtor {
    cfg;
    seen;
    invocation = 0;
    constructor(ctl) {
        super(ctl);
        const ctlAny = (ctl ?? {});
        const wd = (0, controller_js_1.getWorkingDirectory)(ctlAny);
        this.cfg = (0, config_js_1.loadConfig)(wd);
        (0, log_js_1.configure)(this.cfg.logLevel, this.cfg.logFile);
        this.seen = new dedup_js_1.SeenTracker(wd);
        (0, log_js_1.info)("gen", "VisionBridge generator initialized", {
        ...
        const mi = (0, controller_js_1.getModelInfo)(ctlAny);
        if (mi)
            (0, log_js_1.dbg)("gen", "controller model info", { id: mi.id, name: mi.name });
    }
    async *generate(messages, ...rest) {
        this.invocation += 1;
        const inv = `inv${this.invocation}`;
        const msgArr = Array.isArray(messages) ? messages : [];
        (0, log_js_1.info)("gen", `invocation ${inv} started`, {
        ...
        const bridge = (0, vision_bridge_js_1.applyVisionBridge)({ ctl: this.ctl, messages: msgArr, cfg: this.cfg, seen: this.seen });
        ...
        const tools = (0, messages_js_1.toOpenAITools)(toolsArg ?? (0, controller_js_1.getControllerTools)(this.ctl));
        if (tools) {
            (0, log_js_1.dbg)("gen", "forwarding tool definitions to model", {
                count: tools.length,
                names: tools.map((t) => t.function.name),
            });
        }
        else {
            (0, log_js_1.warn)("gen", "no tool definitions found (not in generate() args nor on controller) — model cannot issue MCP tool calls this round");
        }
        const req = { model: this.cfg.model, messages: bridge.messages, stream: true };
        if (tools && tools.length > 0)
            req.tools = tools;
        const queue = new AsyncQueue();
        const runPromise = (0, openai_client_js_1.chatCompletionStream)({ apiRoot: this.cfg.apiRoot, apiKey: this.cfg.apiKey, timeoutMs: this.cfg.timeoutMs }, req, (t) => queue.push(t)).then((result) => {
        ...
        for (;;) {
            ...
            if (item === SENTINEL_END) {
                ...
                if (!done.ok) {
                    (0, log_js_1.err)("gen", `${inv} finished with error`, String(done.error));
                    return;
                }
                const result = done.result;
                (0, log_js_1.info)("gen", `${inv} model output received`, {
                ...
                if (result.toolCalls.length > 0) {
                    for (const tc of result.toolCalls) {
                        ...
                        try {
                            ...
                            (0, log_js_1.warn)("gen", "tool call arguments were not valid JSON", {
                                raw: (tc.function.arguments ?? "").slice(0, 300),
                                error: String(e),
                            });
                        }
                        const lsCall = { id: tc.id, tool: tc.function.name, args };
                        (0, log_js_1.info)("gen", "model requested a tool call — reporting to LM Studio (execution stays with LM Studio)", {
                            id: lsCall.id,
                            tool: lsCall.tool,
                            argKeys: Object.keys(args),
                        });
                        if (!(0, controller_js_1.reportToolCall)(this.ctl, lsCall)) {
                            (0, log_js_1.err)("gen", "tool call could not be reported to LM Studio — the MCP loop cannot continue; see log for the controller surface");
            ...
            if (item.startsWith(SENTINEL_ERR)) {
                ...
                (0, log_js_1.err)("gen", `${inv} failed`, msg);
...
exports.VisionBridgeGenerator = VisionBridgeGenerator;
